...
```{r}
allmarker_files <- sort(unlist(strsplit(params$allmarkers_collected, ",")))
cluster_files <- sort(unlist(strsplit(params$clusters_collected, ",")))
configs <- fread(params$configs_path)
marker_configs <- fread(params$marker_configs_path)
...
get_normalized_cluster_summary <- function(roi_allmarkers, roi_clusters) {
  ...
  cluster_medians <- 
    inner_join(clusters, normcounts, by = c("x" = "Centroid X um", "y" = "Centroid Y um", "roi" = "roi")) %>% 
...
for (i in seq_along(metacluster_range)) {
  ...
  p <- ggplot(meta_orig_tab, aes(x = factor(metacluster, levels = as.character(1:n_metaclusters)), y = Freq, fill = ROI)) +
    ...
    # geom_text(data = combined_counts_by_roi[combined_counts_by_roi$Freq != 0,], mapping = aes(label = `Functional Markers`), position = position_stack(vjust = 0.5), size = 3) +
    theme_bw() +
    xlab("metacluster")
