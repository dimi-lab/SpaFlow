...
all_coords <- fread(allmarkers_outfile)
all_coords <- all_coords  %>% select(roi, `Centroid X um`, `Centroid Y um`)
```
...
clusters_out <- left_join(all_coords, clusters_out, by = c("Centroid X um" = "x", "Centroid Y um" = "y")) %>%
  select("x" = "Centroid X um", "y" = "Centroid Y um", seurat_clusters)
