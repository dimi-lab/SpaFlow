...
if(nrow(marker_configs) > 0 & all(sapply(marker_configs$marker, function(x) any(grepl(x, colnames(roi_df)))))) {
  ...
  markers_selected <- sapply(grep(cluster_metric, colnames(roi_df), value = T), function(x) sub(paste0(": Cell: ", cluster_metric), "", x))
...
get_normalized_counts <- function(roi_allmarkers) {
  ...
  cols <- grep(cluster_metric, colnames(counts), value = T)
...
get_normalized_cluster_summary <- function(roi_allmarkers, roi_clusters) {
  ...
  cluster_medians <- 
    ...
    select(x, y, imageid, kmeans, all_of(paste0(markers_selected, ": Cell: ", cluster_metric))) %>% 
    group_by(kmeans) %>% 
    summarise_at(paste0(markers_selected, ": Cell: ", cluster_metric), list(mean = mean, median = median, sd = sd), na.rm = T) %>%
...
metric_to_exclude <- ifelse(cluster_metric == "Median", "_mean", "_median")
all_normalized_cluster_summaries <- all_normalized_cluster_summaries %>% select(!contains("sd") & !contains(metric_to_exclude))
colnames(all_normalized_cluster_summaries) <- sapply(colnames(all_normalized_cluster_summaries), function(x) sub(paste0("_", tolower(cluster_metric)), "", x))
```
