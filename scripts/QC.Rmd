...
# Remove rows with NAs in marker quant
if (any(is.na(means_only_df))) {
  roi_df <- roi_df[complete.cases(means_only_df)]
  means_only_df <- roi_df %>% select(contains("Cell: Mean"))
  cat("\n***WARNING: Rows containing NaN values have been removed from your dataset. Please double-check your quantification files.***", file="nacheck.txt")
} else {
  cat("", file="nacheck.txt")
}
