...
# allmarkers_outfile <- "output_tables/all_markers_clean_reg002.csv"
# roi <- tools::file_path_sans_ext(basename(allmarkers_outfile))
# roi <- sub("all_markers_clean_", "", roi)
# configs_path <- "configs.csv"
```


```{r}
prior_marker_info <- fread('celesta_prior_matrix.csv')
...
imaging_data <- imaging_data %>% filter(qc == "Cell") %>% select(X = `Centroid X um`, Y = `Centroid Y um`, contains("Cell: Median"))

colnames(imaging_data) <- sapply(colnames(imaging_data), function(x) sub(": Cell: Median", "", x))
