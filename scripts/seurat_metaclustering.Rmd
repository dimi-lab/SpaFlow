...
get_normalized_cluster_summary <- function(roi_allmarkers, roi_clusters) {
  ...
  cluster_medians <- 
    ...
    select(x, y, roi, seurat_clusters, all_of(paste0(markers_selected, ": Cell: ", cluster_metric))) %>% 
...
for (i in seq_along(metacluster_range)) {
  ...
  for (j in cluster_files) {
    ...
    colnames(meta_mapped)[colnames(meta_mapped) == "metacluster"] <- paste0("seurat_metaclusters_", metacluster_range[i])
