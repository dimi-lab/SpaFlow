---
title: "Leiden Metaclustering for MxIF"
...
get_normalized_cluster_summary <- function(roi_allmarkers, roi_clusters) {
  ...
  cluster_medians <- 
    ...
    filter(leiden != "Artifact") %>%
    select(x, y, imageid, leiden, all_of(paste0(markers_selected, ": Cell: ", cluster_metric))) %>%  
    group_by(leiden) %>% 
    summarise_at(paste0(markers_selected, ": Cell: ", cluster_metric), list(mean = mean, median = median, sd = sd), na.rm = T) %>%
    mutate(leiden = paste0(imageid, "_", leiden), imageid = imageid)
...
make_heatmap <- function(dat, n_metaclusters) {
  ...
  mat2 <- smSub %>%
    select(-c(leiden, imageid)) %>% replace(is.na(.), 0) %>%
...
fwrite(centroids_out, "arcsin_zscore_leiden_centroids.csv")

metric_to_exclude <- ifelse(cluster_metric == "Median", "mean", "median")
...
for (i in seq_along(metacluster_range)) {
  ...
  get_metacluster_table <- function(metacluster) {
    ...
    dat <- data.frame(metacluster = metacluster, cluster = smSub$leiden[metaclust_ind], imageid = smSub$imageid[metaclust_ind])
    dat
  }
  ...
  all_metacluster_counts$metacluster <- paste0("leiden_meta_", all_metacluster_counts$metacluster)
  
  for (j in cluster_files) {
    single_df <- df_list[[basename(j)]]
    single_df$cluster <- paste(single_df$imageid, single_df$leiden, sep = "_")
    meta_mapped <- left_join(single_df, all_metacluster_counts, by = c("cluster" = "cluster", "imageid" = "imageid")) #%>%
      #select(x, y, imageid, leiden, cluster, metacluster)
    meta_mapped$metacluster[grepl("Artifact", meta_mapped$cluster)] <- "Artifact"
    colnames(meta_mapped)[colnames(meta_mapped) == "metacluster"] <- paste0("leiden_metaclusters_", metacluster_range[i])
    df_list[[basename(j)]] <- meta_mapped
    
  }
  
  for (i in seq_along(df_list)) {
    file_name <- sub("scimap_clusters", "leiden_metaclusters", names(df_list[i]))
